...
Sting* Sting::create(GameScene* game,int theType, float theStingAngle)
{
    Sting *pRet = new Sting();
    if (pRet && pRet->init(game,theType,theStingAngle))
    {
        pRet->autorelease();
        return pRet;
    }
...
bool Sting::init(GameScene* game,int theType, float theStingAngle)
{
	if (!CCNode::init())
	{
		return false;
...
//        p = CCParticleSystemQuad::create("Particles/bullet_part_red.plist");
    ...
    */
	...
	_bullet = CCSprite::create("Characters/bee/dzelonis2.png");
	
	_bullet->setRotation(_rotationAngle);
	
   // _animation->retain();
    //_animation->setOpacity(0);
    addChild(_bullet, kHUD_Z_Order-1);
    
    CCFadeIn* aFadeIn = CCFadeIn::create(0.3f);
    _bullet->runAction(aFadeIn);
	
	return true;
}

void Sting::OnDoAction(Dwarf* theForced)
{
    _isDisabled = true;
    _canMove = false;
...
//    CCLog("The Angle at bullet in loop: %f",_angle);
    ...
    if(_straightCords.x > 0){
        //Check if it is not null

        
        CCPoint point = _straightCords;
...
float Sting::GetAngle()
{
    return _angle;
}

void Sting::setAngle(float value)
{
	_angle = wrapTwoPI(value);
}

bool Sting::ccTouchBegan(cocos2d::CCTouch* touch, cocos2d::CCEvent* event)
{
    if(_isDisabled){
        return false;
    }
...
void Sting::ccTouchMoved(cocos2d::CCTouch* touch, cocos2d::CCEvent* event)
{
    //Do nothing for now !!!
    
}

void Sting::ccTouchEnded(cocos2d::CCTouch* touch, cocos2d::CCEvent* event)
{
    if(_isDisabled){
        return;
    }
